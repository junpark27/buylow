...
interface ToolbarAction {
  ...
  prefix: string;
  suffix: string;
  block?: boolean; // inserts on new line
}
...
const AUTOSAVE_KEY = "buylow-editor-draft-new";
...
export default function NewPostPage() {
      ...
      if (title || content) {
        try {
          localStorage.setItem(
            AUTOSAVE_KEY,
            JSON.stringify({ title, description, tags, content })
          );
          const now = new Date();
          setLastSaved(
  ...
  const insertMarkdown = useCallback(
    (action: ToolbarAction) => {
      ...
      // Restore cursor position after the inserted prefix + selected text
